...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setParent () {
        // 给每个节点添加一个完整路径的配置，用于完整匹配搜索
        const nodeData = this.$refs.authTree.store.nodesMap
        for (let key in nodeData) {
          if (nodeData.hasOwnProperty(key)) {
            const data = nodeData[key].data
            let parent = this.$refs.authTree.getNode(data.auth_pid) ? this.$refs.authTree.getNode(data.auth_pid).data : null
            let pathName = [data.auth_name]
            let pathId = [data.auth_id]
            while (parent && parent.auth_id) { // 刨根问底，一直找到该节点所有祖先节点名称
              pathName.unshift(parent.auth_name)
              pathId.unshift(parent.auth_id)
      ...
      moveAuth (data, parent) {
        ...
        this.moveTarget.parentName = data.pathName
      ...
      replaceSearch (data) {
        if (data.pathName === this.localSearchValue || data.pathId === this.localSearchValue
          ...
          return `<font class='find-sreach-world' style='background: #ffff00;'>${data.auth_name + (+data.type === 1 ? '' : '（权限码：' + data.auth_code + '）')}</font>`
        } else if (data.auth_name.indexOf(this.localSearchValue) !== -1 || data.auth_code.indexOf(this.localSearchValue) !== -1) {
          return `<font class='find-sreach-world' style='background: #ffff00;'>${data.auth_name + (+data.type === 1 ? '' : '（权限码：' + data.auth_code + '）')}</font>`
        } else {
      ...
      3、getSreachResult
        a、通过遍历所有树节点数据，存在匹配数据，记录下该节点的id，加入到展开列表中（$refs.refName.store.nodesMap）
          (1)后增加 路径全匹配
          (2)setParent 为每个节点添加一个完整路径的属性值，通过 祖级/父级/子级…… 的方式进行匹配，采用全匹配和字符串结尾匹配方式
          (3)该情况下不能做包含处理，否则搜索父级，所有子级都会被判断为匹配
          (4)判断是否存在‘/’，存在先进行全匹配，若无结果进行字段匹配（可能存在名称本身带有/的情况）
